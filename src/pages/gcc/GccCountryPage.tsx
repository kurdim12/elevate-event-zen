...
import { getGccCountry, GccLang } from "@/data/gcc-countries";

interface GccCountryPageProps {
  lang: GccLang;
}

export default function GccCountryPage({ lang }: GccCountryPageProps) {
  const { countrySlug } = useParams<{ countrySlug: string }>();
  const country = countrySlug ? getGccCountry(countrySlug) : undefined;

  if (!country) {
    return <Navigate to="/" replace />;
  }

  const content = country[lang];
  const isAr = lang === "ar";

  const enPath = `/gcc/${country.slug}`;
  const arPath = `/ar/gcc/${country.slug}`;
  const canonicalPath = lang === "ar" ? arPath : enPath;
  ...
  const contactPath = isAr ? "/contact" : "/contact";
